<%@page import="com.boot.pojo.readlater"%>
...
<body>
	...
	<%
		...
		<h2>
			Welcome&nbsp;<%=session.getAttribute("mail")%><%} %>
			 <a href="<%=request.getContextPath()%>/likedbooks" class="lmove"><button
				...
				</button></a> <a href="viewbooks"><button
					class="glow-on-hover">
					<i class="fas fa-book" style="color: green;"></i><span class="tooltiptext"
						style="color: green;">Dashboard</span>
		...
		<div class="contentsdisp">
			<%
			List<readlater> bookies = (List<readlater>) request.getAttribute("books");

			for (readlater book : bookies) {
			%>
			<div class="backg">
				<div class="center">
					<img class="imgedit"
						src="assets/images/<%=book.getTitle()%>.jpg" />
				</div>
				<p class="details">
				...
				<p class="lrbutton">
					<a
						href="<%=request.getContextPath()%>/deletereadlater?value=<%=book.getId()%>"><Button
							type="submit" id="" class="rbstyle">Remove</Button></a>
				</p>
			</div>
			<%} %>
		</div>
